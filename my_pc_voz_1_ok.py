...
class ChatbotApp:
    def __init__(self, root):
        ...
        self.voice_label = tk.Label(root, text="Selecionar voz:")
    ...
    def toggle_recording(self, event=None):
        if self.is_recording:
            self.stop_recording()
            self.record_button.config(text="Clique para gravar", bg='blue')
        else:
    ...
    def get_response(self, user_message):
        try:
            ...
            if json_response.get('type') == 'normal':
                ...
                print("Clique:", json_response.get('content'))
                self.click_on(json_response.get('content'))
                return json_response.get('content')

        except Exception as e:
            return f"Desculpe, não foi possível obter uma resposta. Por favor, tente novamente mais tarde. Erro: {e}"
    ...
    def speak_response(self, response_text):
        try:
            ...
            print("Erro durante a conversão texto-em-voz:", e)
